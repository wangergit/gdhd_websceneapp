...
define([
  ...
  function(declare, lang, array, html, when, on, /*aspect,*/ query, keys, Deferred, all,
    BaseWidget, /*LayerInfos,*/ jimuUtils, Search, SearchViewModel,SearchSource, Locator, FeatureQuery,
    FeatureLayer, SceneLayer, /*InfoTemplate,*/ PictureMarkerSymbol, PopupTemplate, utils,esriRequest,GeometryEngine,Graphic,Point,Polygon,Polyline) {
    ...
    return declare([BaseWidget], {
      ...
      startup: function() {
        ...
        when(utils.getConfigInfo(this.config)).then(lang.hitch(this, function(config) {
          ...
          var bridgePath = "https://wanger.scene.com/arcgis/rest/services/gd/Bridge/MapServer";
          var customSearchSource = new SearchSource({
              placeholder: "请输入桥梁名称",
              getSuggestions: function(params) {
                  var code = encodeURI("'%" + params.suggestTerm + "%'");
                  var path = bridgePath + "/0/query?where=NOBJNM like " + code + "&text=&objectIds=&time=&geometry=&geometryType=esriGeometryEnvelope&inSR=&spatialRel=esriSpatialRelIntersects&relationParam=&outFields=*&returnGeometry=true&returnTrueCurves=false&maxAllowableOffset=&geometryPrecision=&outSR=&returnIdsOnly=false&returnCountOnly=false&orderByFields=&groupByFieldsForStatistics=&outStatistics=&returnZ=false&returnM=false&gdbVersion=&returnDistinctValues=false&resultOffset=&resultRecordCount=&queryByDistance=&returnExtentsOnly=false&datumTransformation=&parameterValues=&rangeValues=&f=pjson"
                  return esriRequest(path, {}).then(function(results) {
                      var data = results.data.features.map(function(feature) {
                          return {
                              key: "NOBJNM",
                              text: feature.attributes.NOBJNM,
              ...
              getResults: function(data) {
                  var params = esriRequest(bridgePath + "/0/query?where=OBJECTID = " + data.suggestResult.id + "&text=&objectIds=&time=&geometry=&geometryType=esriGeometryEnvelope&inSR=&spatialRel=esriSpatialRelIntersects&relationParam=&outFields=*&returnGeometry=true&returnTrueCurves=false&maxAllowableOffset=&geometryPrecision=&outSR=&returnIdsOnly=false&returnCountOnly=false&orderByFields=&groupByFieldsForStatistics=&outStatistics=&returnZ=false&returnM=false&gdbVersion=&returnDistinctValues=false&resultOffset=&resultRecordCount=&queryByDistance=&returnExtentsOnly=false&datumTransformation=&parameterValues=&rangeValues=&f=pjson", {
                      query: {},
                      responseType: "json"
                  }).then(function(results) {
                      var searchResults = results.data.features.map(function(feature) {
                          var height = feature.attributes.DHNWNO ? feature.attributes.DHNWNO : 30
                          for(var j = 0 ; j < feature.geometry.rings[0].length ; j ++){
                              feature.geometry.rings[0][j].push(30)
                          }
                          ...
                          var searchResult = {
                              ...
                              feature: graphic,
                              name: feature.attributes.NOBJNM
                          };
                          return searchResult;
                      });
