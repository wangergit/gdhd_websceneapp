  ...
  function(declare, html, _WidgetsInTemplateMixin, Point, SpatialReference, BaseWidget, utils, Message,
    ...
    var clazz = declare([BaseWidget, _WidgetsInTemplateMixin], {
      ...
      _processData: function() {
        ...
        var basemap = this.sceneView.map.basemap.baseLayers.items[0];
      ...
      _displayOnClient: function(mapPoint) {
        ...
        if (!options || options.isGeographicUnit) {
        ...
        if (convertInClient) {
          ...
          if (mapPoint.spatialReference.wkid === 4326 && this._isWebMercator(this.selectedWkid)) {
            ...
            } else if (!options || options.isGeographicUnit) {
              this._displayDegOrDms(outUnit, y, x);
            }
          }
        } else {
          // use default units
          if (!options || options.defaultUnit === outUnit) {
          ...
          if (mapPoint.spatialReference.wkid === 4326 ||
            ...
            if ("MGRS" === outUnit || "USNG" === outUnit) {
              ...
              this._displayDegOrDms(outUnit, y, x);
            } else if (!options || options.isProjectedCS) {
              this._displayProject(outUnit, y, x);
            }
          } else { // proj or geo
            if (!options || options.isProjectedCS) {
              this._displayProject(outUnit, y, x);
            } else if (!options || options.isGeographicCS) {
              if ("MGRS" === outUnit || "USNG" === outUnit) {
      ...
      _displayDegOrDms: function(outUnit, y, x) {
        ...
        options ? x = x * options.unitRate : null;
        options ? y = y * options.unitRate : null;
